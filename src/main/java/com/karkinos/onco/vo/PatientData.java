package com.karkinos.onco.vo;

import java.util.Date;

import javax.validation.constraints.NotBlank;
import javax.validation.constraints.NotNull;

import org.apache.commons.lang3.StringUtils;

import com.karkinos.onco.util.Constants;

import lombok.AllArgsConstructor;
import lombok.Getter;
import lombok.NoArgsConstructor;
import lombok.Setter;

@Getter
@Setter
@NoArgsConstructor
@AllArgsConstructor
public class PatientData {

	public enum BLOODGROUP {
		APLUS("A+"), AMINUS("A-"), BPLUS("B+"), BMINUS("B-"), OPLUS("O+"), OMINUS("O-"), ABPLUS("AB+"), ABMINUS("AB-");

		private String val;

		BLOODGROUP(String val) {
			this.val = val;
		}

		public String getValue() {
			return val;
		}
	}

	private String salutation;

	private String fullName;

	@NotBlank(message = "firstName" + Constants.CANNOT_BE_EMPTY)
	private String firstName;

	private String middleName;

	private String lastName;

	private int age;

	@NotNull(message = "gender" + Constants.CANNOT_BE_EMPTY)
	private String gender;

	private Date dob;

	private String mobile;

	private String email;

	private Address address;

	private double height;

	private double weight;

	private String ABHAAddress;
	
	private Identifier identifier;

	private BLOODGROUP bloodGroup;

	public String getFullName() {
		fullName = (StringUtils.isBlank(getSalutation()) ? "" : (getSalutation() + " ")) + (getFirstName())
				+ (StringUtils.isBlank(middleName) ? "" : (" " + middleName))
				+ (StringUtils.isBlank(getLastName()) ? "" : (" " + getLastName()));
		return fullName != null ? fullName.trim() : fullName;
	}
}
