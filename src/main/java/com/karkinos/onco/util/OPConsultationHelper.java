...
import com.karkinos.onco.model.Doctor;
import com.karkinos.onco.model.Medicine;
import com.karkinos.onco.model.Obs;
import com.karkinos.onco.model.SimpleAllergy;
import com.karkinos.onco.model.SimpleCondition;
import com.karkinos.onco.model.SimpleDiagnosticTest;
...
public class OPConsultationHelper {
	...
	public Date getCompositionDate(LocalDateTime dateTime, int docIndex) {
		return Utils.getFutureDate(dateTime, docIndex * 2);
	}

	public Path getFileSavePath(DocRequest request, Date date, Bundle.BundleEntryComponent patientEntry) {
		String fileName = String.format("%s%s" + getDocBasicName() + "%s.json", request.getHipPrefix().toUpperCase(),
	...
	@SneakyThrows
	public Bundle createOPConsultationBundle(Date date, String patientName, String hipPrefix, IParser jsonParser,
			String patientId, PatientData patientData) {
		...
		if (Utils.randomBool()) {
			Organization organization = jsonParser.parseResource(Organization.class,
					FHIRUtils.loadOrganization(hipPrefix));
...
//		Patient patientResource = FHIRUtils.getPatientResource(patientName, patientId, patients);
		Patient patientResource = patientService.patientGenerator(patientData);
	...
	protected void createProcedureSection(Bundle bundle, Composition composition, Patient patient, String hipPrefix) {
		...
		if (Utils.randomBool()) {
			procedure.setCode(FHIRUtils.getCodeableConcept("90105005", "Biopsy of soft tissue of forearm (Procedure)",
					"Biopsy of suspected melanoma L) arm"));
		} else {
			procedure.setCode(FHIRUtils.getCodeableConcept("232717009", "Coronary artery bypass grafting", ""));
		}
...
//		performer.setActor();
		...
		if (Utils.randomBool()) {
			...
			Resource existingResourceInBundle = FHIRUtils.findResourceInBundleById(bundle, ResourceType.Practitioner,
					surgeon.getId());
		...
		procedure.setComplication(
				Collections.singletonList(FHIRUtils.getCodeableConcept("131148009", "Bleeding", null)));
	...
	@SneakyThrows
	protected void createDiagnosticReportSection(Bundle bundle, Composition composition, Patient patient,
			IParser jsonParser, String hipPrefix) {
		if (Utils.randomBool())
			return; // dont need diagnosticReport always
		...
		if (Utils.randomBool()) {
			DocumentReference docReference = FHIRUtils.getReportAsDocReference(interpreter,
	...
	protected void createDocumentsSection(Bundle bundle, Composition composition, Patient patient, String hipPrefix) {
		...
		if (Utils.randomBool()) {
			Practitioner anotherDoc = FHIRUtils.createAuthor(hipPrefix, doctors);
			Practitioner doctor = (Practitioner) FHIRUtils.findResourceInBundleById(bundle, ResourceType.Practitioner,
					anotherDoc.getId());
			if (doctor == null) {
	...
	protected void createPrescriptionSection(Bundle bundle, Composition composition, Patient patient) {
		...
		for (int i = 0; i < numberOfMeds; i++) {
			...
			MedicationRequest medReq = FHIRUtils.createMedicationRequest(getPractitioner(composition),
					getMedicationDate(composition), med, medication, condition, useMedicationCodeableConcept);
	...
	protected void createChiefComplaintsSection(Bundle bundle, Composition composition, Patient patient) {
		...
		for (int i = 0; i < numberOfComplaints; i++) {
			Condition condition = FHIRUtils.createCondition(SimpleCondition.getRandomComplaint(),
					getComplaintDate(composition));
			condition.setSubject(referenceToPatient);
			FHIRUtils.addToBundleEntry(bundle, condition, false);
