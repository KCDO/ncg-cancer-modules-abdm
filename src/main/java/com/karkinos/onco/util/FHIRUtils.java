...
import com.karkinos.onco.model.Vaccine;
...
public class FHIRUtils {
	...
	static Patient getPatientResource(String name, String patientId, Properties patients) throws Exception {
		...
		com.karkinos.onco.model.Patient patient = com.karkinos.onco.model.Patient.parse((String) patientDetail);
	...
	static CodeableConcept getDiagnosticReportType() {
		...
		coding.setSystem(Constants.SCT_SYSTEM);
	...
	static Medication getMedication(Medicine med) {
		...
		if (Utils.randomBool()) {
			...
			coding.setSystem(Constants.ACT_SYSTEM);
	...
	static MedicationRequest createMedicationRequest(Practitioner author, Date date, Medicine med,
			Medication medication, Condition condition, boolean useMedicationCodeableConcept) {
		...
		if (useMedicationCodeableConcept) {
			...
			var text = medication.getCode().hasText() ? medication.getCode().getText()
	...
	static Condition createCondition(SimpleCondition randomComplaint, Date date) {
		...
		condition.setId(UUID.randomUUID().toString());
		if (Utils.randomBool()) {
			condition.setClinicalStatus(conceptWith(randomComplaint.getClinicalStatus(),
					randomComplaint.getClinicalStatus(), Constants.FHIR_CONDITION_CLINICAL_STATUS_SYSTEM));
		}
		condition.setCode(conceptWith(randomComplaint.getText(), randomComplaint.getCode(), Constants.SCT_SYSTEM));
		condition.setCategory(Collections.singletonList(conceptWith(randomComplaint.getCategory(),
				randomComplaint.getCategoryCode(), Constants.FHIR_CONDITION_CATEGORY_SYSTEM)));
		condition.setSeverity(
				conceptWith(randomComplaint.getSeverity(), randomComplaint.getSeverityCode(), Constants.SCT_SYSTEM));
		if (Utils.randomBool()) {
			condition.setRecordedDate(date);
		}
		if (Utils.randomBool()) {
			Date onsetDate = Utils.getPastDate(date, 30);
			if (Utils.randomBool()) {
